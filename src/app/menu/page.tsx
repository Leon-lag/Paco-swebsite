      ...
      <div className="mt-12 space-y-12">
        {menuCategories.map((category) => (
          <section key={category.name}>
            <h2 className="mb-6 text-2xl font-bold text-amber-700 dark:text-amber-400 border-b border-amber-200/60 pb-3 dark:border-amber-900/40">
              {category.name}
            </h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {category.items.map((item) => (
                <div
                  key={item.name}
                  className="rounded-xl border border-amber-200/60 p-5 dark:border-amber-900/40 hover:border-amber-300 dark:hover:border-amber-700 transition-colors"
                >
